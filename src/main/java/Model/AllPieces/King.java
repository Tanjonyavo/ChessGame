...
public class King extends Piece{
    ...
    public boolean isValidMovement(int col, int row) {
        return Math.abs(col - this.col) <= 1 && Math.abs(row - this.row) <= 1 || canCastle(col, row);
    }
            ...
            if(col == 6){
                ...
                if(rook != null && rook.isFirstMove && isFirstMove){
                    return board.getPiece(5, row) == null && board.getPiece(6, row) == null &&
                            !board.checkScanneur.isKingChecked(new Move(board, this, 5, row));
                    }
                }else if(col == 2){
                Piece rook = board.getPiece(0, row);
                if(rook != null && rook.isFirstMove && isFirstMove){
                    return board.getPiece(3, row) == null && board.getPiece(2, row) == null &&
                            board.getPiece(1, row) == null &&
                            !board.checkScanneur.isKingChecked(new Move(board, this, 3, row));
                }
            }
        }
        return false;
